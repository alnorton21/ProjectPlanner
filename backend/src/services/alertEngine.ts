...
function getConfig(configs: AlertConfig[], type: string): AlertConfig | undefined {
  return configs.find(c => c.alert_type === type && c.enabled === 1);
}

export function runAlertEngine(projectId: number): void {
  const db = getDb();

  const configs: AlertConfig[] = db.prepare('SELECT * FROM alert_config').all() as AlertConfig[];
  const tasks: TaskRow[] = db.prepare(
    'SELECT id, project_id, task_name, bucket_name, progress, progress_percent, assigned_to, due_date FROM tasks WHERE project_id = ?'
  ).all(projectId) as TaskRow[];

  // Clear non-dismissed alerts for this project
  db.prepare("DELETE FROM alerts WHERE project_id = ? AND is_dismissed = 0").run(projectId);

  const insert = db.prepare(`
    INSERT INTO alerts (project_id, task_id, alert_type, severity, message)
    VALUES (@project_id, @task_id, @alert_type, @severity, @message)
  `);
  ...
  if (behindConfig) {
    const project = db.prepare('SELECT * FROM projects WHERE id = ?').get(projectId) as { imported_at: string } | undefined;
    const allDueDates = tasks.filter(t => t.due_date).map(t => parseISO(t.due_date!));
    if (project && allDueDates.length > 0) {
